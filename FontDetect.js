var FontDetect = (function(){
    ...
    function isMono(f){
        ...
        for( i = 1; i < testText.length; i++ ){
            if( Math.abs(ctx.measureText(testText.charAt(i)).width-w) > fudge ) return false;
        }
        return true;
    }
    ...
    function loadSWF(cb){
        if( hasFlash ){
            _timer = window.setTimeout(cb,timeout);
            _cb = cb;
            _swfObj = document.createElement("object");
            _swfObj.data = SWFPath+"?onLoad=FontDetect.swfFonts";
